...
**Implementation (Sep 2026).** The Dashboard rows and each project's Jobs table (`app/projects/[id]/jobs-table.tsx`; the standalone `/jobs` board was folded into the project page) are rendered by one shared list surface, `components/ap/data-table.tsx`: TanStack Table v9 for sorting, a per-column facet filter and a search box, over the restyled shadcn primitives in `components/ui/` (`table`, `tabs`, `input`, `skeleton`). Column headers are the sort controls (mono label plus a 14px chevron, `aria-sort` on the cell), facets are underline tabs on a hairline rail, the whole row is a click target with a real link in the first cell, and the filtered-empty state follows §9.9 with a ghost "Clear filters" action. New dashboard-style lists should compose this rather than hand-roll rows. (A project's Crew section is the one deliberate exception: its rows are one-line summaries that expand in place into the contractor's profile, `app/projects/[id]/crew-list.tsx`, so a table with sortable columns would fight the disclosure.) The Jobs table carries two status axes per row: the user's own status (`orders.job_status`: ACTIVE / PENDING / DONE), set in place through `components/ap/job-status-select.tsx`, a token-shaped native select (tone dot, mono label, hairline pill, 14px chevron) that the facet tabs filter on, and the vendor-driven Confirmation token (PLACED / PROCESSING / CONFIRMED), read-only.
